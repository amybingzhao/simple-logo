...
public class GUICanvas implements Observer {
	...
	private boolean updateAllTurtles;
	...
	public void setRightCanvas() {
		canvasRight = new GUICanvasRight(myResources,
				new GUIComboBoxColorB(this, myResources, myResources.getString("BackgroundColorPalettePromptText"),
						myResources.getString("DefaultBackgroundColors")),
				new GUIComboBoxColorP(this, myResources, myResources.getString("PenColorPalettePromptText"),
	...
	/**
	 * Resets Canvas. Deletes all of the Turtle's trails and changes the Turtle
	 * back to default.
	 */
	private void resetCanvas(Turtle turtle) {
		for (Turtle key : myTurtles.keySet()) {
			...
			drawTurtle(key);
			myTurtleImageView.updateImageView(turtle.getImageView(), STARTING_X, STARTING_Y, DEFAULT,
					myTurtleImageView.getTurtleShape(), true);
			key.doneResetting();
		}
	}
	...
	private void setOldCoordinates(Turtle turtle, double x, double y, double direction) {
		...
		if (turtleParameters.size() <= turtle.getID())
			...
			turtleParameters.set((int) turtle.getID() - 1, coordinates);
	}
	...
	private void addTurtleToMap(Turtle turtle) {
		if (!myTurtles.containsKey(turtle)) {
			...
			myTurtles.put(turtle,
					new ArrayList<GraphicsContext>(Arrays.asList(turtleCanvas.getGraphicsContext2D(), drawingGC)));
			int myX = STARTING_X;
			int myY = STARTING_Y;
	...
	/**
	 * Draws the turtle onto canvas based on turtle's X and Y values and its
	 * direction.
	 */
	...
	private void checkForAnimation(Turtle turtle, double myX, double myY) {
		...
		if (myAnimation.willAnimate()) {
			myAnimation.addAnimation(currentImageView, normalizeCoordinates(parameters[0]),
					normalizeCoordinates(parameters[1]), normalizeCoordinates(myX), normalizeCoordinates(myY),
					turtle.getDirection() - parameters[2]);
			myTurtleImageView.updateImageViewLocation(currentImageView, myX, myY, myTurtleImageView.getTurtleShape(),
					updateAllTurtles);
		} else {
			myTurtleImageView.updateImageView(currentImageView, myX, myY, turtle.getDirection(),
					myTurtleImageView.getTurtleShape(), updateAllTurtles);
		}
	}

	/**
	 * Converts coordinate to its toroidal coordinate if it is greater than the
	 * maximum or less than the minimum coordinate.
	 * 
	 * @param coordinate
	 * @return
	 */
	...
	private void drawOval(GraphicsContext gcDrawing, double myX, double myY) {
		...
		gcDrawing.fillOval(myX + CENTER_FACTOR - penSize / 2, myY + CENTER_FACTOR - penSize / 2, penSize, penSize);
	}
	...
	/**
	 * Update both pen and background color palette at given index to given RGB
	 * color.
	 * 
	 * @param Space
	 *            separated string of RGB values
	 * @param index
	 *            of color in palette that user wants to change
	 */
	...
	public void setUpdateAllTurtles(boolean update) {
		updateAllTurtles = update;
	}
	
//	protected GUICanvasTurtleImageView getView() {
//		return myTurtleImageView;
//	}
}